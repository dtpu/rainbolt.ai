...
![Photo view: every clue the model used is pinned on the analyzed image with sourced reference imagery per clue; users drop their own pins and ask about them, and clue mentions in chat link back to the pins](images/annotate.png)
...
4. Gemini reasoning. The candidate coordinates and matched feature clues are handed to Gemini (via LangChain, `langchain_google_genai`). Gemini reasons over the evidence, narrows the location, and explains its thinking. The reasoning is streamed token by token to the client over a WebSocket so the UI can show the model working in real time. A follow-up multimodal pass pins the strongest visual clues onto the photo itself (fractional coordinates), re-run whenever a chat exchange revises the guess. Coordinate/clue JSON is parsed defensively (strict parse, then Python-literal, then `json_repair`), and quota exhaustion on the primary model falls back to `gemini-2.5-flash-lite` with backoff, so a free-tier key degrades gracefully instead of failing.
...
6. Presentation. The Next.js frontend (responsive down to phones) renders the prediction on an interactive globe and constellation view, shows the streamed reasoning with sourced photo evidence for each candidate (Wikimedia Commons geosearch), and displays the street-view imagery. The centre stage flips between the globe and an annotated Photo view: every clue the model used is pinned on the image, opening a pin shows sourced reference imagery of that clue (Commons search) plus a verify link, clue mentions in chat are links that jump to their pin, and users can drop their own pins - notes ride along as context so the model can answer questions about them. A Places tab lists the ranked candidates alongside the user's own geocoded hunches (OpenStreetMap Nominatim) for comparison. Guests can run full sessions without an account (unsaved), seeded from bundled sample photos.
