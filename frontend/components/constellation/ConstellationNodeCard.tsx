...
export const ConstellationNodeCard: React.FC<{
  ...
  ({
    ...
    return (
      ...
      >
        <div
          className={`group relative rounded-xl border bg-space-900/80 p-4 backdrop-blur-md transition-[border-color,box-shadow] duration-300 ${borderClass}`}
        >
            ...
            {!isSettingsOpen ? (
              ...
              <div className="flex items-center gap-0.5 rounded-md border border-white/10 bg-space-800 p-0.5">
                ...
                <button
                  ...
                  title={
                    linkCopied ? "Link copied!" : "Link to another session"
                  }
                  aria-label={
                    linkCopied ? "Link copied" : "Link to another session"
                  }
