...
type SocketWithTimeout = WebSocket & { sendingTimeout?: ReturnType<typeof setTimeout> };
...
function handleSocketMessage(event: MessageEvent, { get, set }: StoreApi) {
    ...
    if (data.type === 'status') {
        ...
        try {
            ...
            const newMarkers: Marker[] = coordinates.map((coord: CoordinatePayload) => ({
...
function attachSocketHandlers(ws: WebSocket, api: StoreApi, reject: (reason?: unknown) => void) {
            ...
            connectWebSocket: (sessionId: string) => {
                return new Promise<void>((resolve, reject) => {
                    ...
                    const shouldSkipProcessing =
                        (state.sessionId === sessionId && state.hasProcessedSession) || hasExistingData;
                    ...
                    ws.onopen = () => {
                        ...
                        ws.send(JSON.stringify({ type: 'process_image', session_id: sessionId }));
