...
- **`features` namespace** — GeoGuessr-style text clues (bollards, road lines,
  ...
  image<->text scores are low (~0.19-0.28), so the feature thresholds in main.py
...
1. **Credentials** in `backend/.env`:
   - `PINECONE_API_KEY` — already restored from git history; that account is live,
     ...
     Pinecone account — free tier is plenty for ~10k vectors.)
   - `KAGGLE_USERNAME` / `KAGGLE_KEY` — the only new credential you must supply.
     Get it at kaggle.com → Account → Create New Token (downloads `kaggle.json`).
...
3. **Run the ingestion** (start small to validate the whole path):
   ...
   **Resumable.** Each completed shard is recorded in
   ...
   - Goal ~1M: that's ~a full day of CPU embedding total, and it may hit
